# Fallout: Thunderbirds — Player Advancement & Perks Instructions

This document defines:

- How characters advance in level
- How Hit Points and skill points increase on level up
- How skill progression costs work
- How perks function, including ranks and requirements

Robots **never** gain perks, even special ones.

---

## Advancement & Experience

Characters gain experience points (XP) by defeating enemies and completing tasks. As XP accumulates, characters gain levels.

### Level & XP Table

| Level | XP Required (Total)       |
|------:|---------------------------|
| 1     | 0                         |
| 2     | 1,000                     |
| 3     | 3,000                     |
| 4     | 6,000                     |
| 5     | 10,000                    |
| 6     | 15,000                    |
| 7     | 21,000                    |
| 8     | 28,000                    |
| 9     | 36,000                    |
| 10    | 45,000                    |
| 11    | 55,000                    |
| 12    | 66,000                    |
| 13    | 78,000                    |
| 14    | 91,000                    |
| 15    | 105,000                   |
| 16    | 120,000                   |
| 17    | 136,000                   |
| 18    | 153,000                   |
| 19    | 171,000                   |
| 20    | 190,000                   |
| 21    | 210,000                   |
| 22+   | +40,000 XP per new level  |

For levels 22 and higher, each new level requires **40,000 more XP** than the previous level.

---

## Hit Points & Skill Points on Level Up

On gaining a level:

Let:
- `END` = Endurance
- `IN` = Intelligence

### Hit Points Gained

Each level, a character gains:

> HP Gain = 3 + floor(END / 2)

This is added to maximum Hit Points.

### Skill Points Gained

Each level, a character gains:

> SkillPoints = 5 + (2 × IN)

Skill points are then spent on skills according to the Skill Progression table below.

---

## Skill Progression Costs

Not all skills increase at the same rate. The cost in skill points to raise a skill by 1% depends on its current value.

| Current Skill % | SP Cost per +1% |
|----------------:|-----------------|
| 1–100           | 1 SP            |
| 101–125         | 2 SP            |
| 126–150         | 3 SP            |
| 151–175         | 4 SP            |
| 176–200         | 5 SP            |
| 201+            | 6 SP            |

---

## Perks Overview

Every few levels, characters gain **perks**.

- Perks represent knowledge, experience, and special advantages gained in the wasteland.
- Perks may affect:
  - Primary Statistics
  - Secondary Statistics
  - Skills
  - Combat rules
  - Other game systems
- Some perks have **ranks**:
  - A perk with 2 ranks can be chosen at most twice, etc.
- Some perks require:
  - Minimum attribute scores
  - Minimum skill values
  - A minimum level
  - Specific race or type (e.g., Ghoul, Mutant, Deathclaw)
- **Special perks** may exist that are not listed here; they are rare and should be treated as significant.

> **Robots never gain perks**, even special ones.

---

## Perk List

Each perk below is listed with:
- A short description
- Effects
- Ranks
- Requirements
- Restrictions (if any)

---

### Action Boy / Action Girl

Make the most of every moment in combat.

- **Effects:** +1 Action Point per rank, each combat turn
- **Ranks:** 2
- **Requirements:** Agility 5, Level 12

---

### Adrenaline Rush

Fear of death makes you stronger when wounded.

- **Effects:** When HP < 50% of maximum, +1 Strength (up to racial max)
- **Ranks:** 1
- **Requirements:** Strength 1–9, Level 6

---

### Animal Friend

Animals see you as one of their own.

- **Effects:** Animals will not attack unless threatened or attacked first
- **Ranks:** 1
- **Requirements:** Intelligence 5, Outdoorsman 25%, Level 9

---

### Awareness

You always know what’s going on in a fight.

- **Effects:** On examining a critter, you see its exact Hit Points and equipped weapon
- **Ranks:** 1
- **Requirements:** Perception 5, Level 3

---

### Bend the Rules

Next time, you get to ignore the rules.

- **Effects:** On your next perk choice, you may ignore all perk restrictions **except race**
- **Ranks:** 1
- **Requirements:** Luck 6, Level 16

---

### Better Criticals

Your critical hits are especially brutal.

- **Effects:**
  - Critical hits deal 150% of normal damage (after other bonuses)
  - Chance to damage a limb is increased by 50%
- **Ranks:** 1
- **Requirements:** Perception 6, Luck 6, Agility 4, Level 9  
- **Restrictions:** Mutants cannot choose this perk

---

### Bluff Master

You can talk your way out of trouble.

- **Effects:** When caught stealing, you automatically talk your way out
- **Ranks:** 1
- **Requirements:** Charisma 3, Level 8

---

### Bone Head

Your skull is remarkably hard.

- **Effects:**
  - Rank 1: 50% chance to avoid being knocked unconscious
  - Rank 2: 75% chance to avoid being knocked unconscious
- **Ranks:** 2
- **Requirements:** Strength 7, Level 7

---

### Bonsai

You have a fruit tree growing from your head.

- **Effects:** Steady supply of fruit (details up to GM)
- **Ranks:** 1
- **Requirements:** Outdoorsman 50%, Science 40%, Level 12  
- **Restrictions:** Only Ghouls may choose this perk

---

### Bonus HtH Attacks

You can make more melee attacks per turn.

- **Effects:** HtH and melee attacks cost 1 less AP
- **Ranks:** 1
- **Requirements:** Agility 6, Level 15

---

### Bonus HtH Damage

You hit harder in close combat.

- **Effects:** +2 Melee Damage per rank
- **Ranks:** 3
- **Requirements:** Agility 6, Strength 6, Level 3

---

### Bonus Move

You move farther for free.

- **Effects:** Per rank, first 2 hexes of movement each turn cost 0 AP; total +2 hexes of free movement
- **Ranks:** 2
- **Requirements:** Agility 5, Level 6

---

### Bonus Ranged Damage

You do extra damage with ranged weapons.

- **Effects:** +2 damage per bullet for each rank
- **Ranks:** 2
- **Requirements:** Agility 6, Luck 6, Level 6  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Bonus Rate of Fire

Your trigger finger is fast.

- **Effects:** All ranged weapon attacks cost 1 less AP
- **Ranks:** 1
- **Requirements:** Agility 7, Intelligence 6, Perception 6, Level 15  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Bracing

You know how to brace big weapons.

- **Effects:** Gain a bonus when bracing large weapons (e.g., simulated tripod) while standing
- **Ranks:** 1
- **Requirements:** Strength 7, Guns 80%, Level 4  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Break the Rules

Next time, no rules apply.

- **Effects:** On your next perk choice, you may choose any perk regardless of requirements or race
- **Ranks:** 1
- **Requirements:** Luck 6, Level 20

---

### Brown Noser

You’re good at sucking up to authority.

- **Effects:** +1 Charisma for reaction rolls with authority figures, per rank
- **Ranks:** 2
- **Requirements:** Charisma 5, Intelligence 6, Level 2  
- **Restrictions:** Dogs and Deathclaws cannot choose this perk

---

### Brutish Hulk

You gain extra HP per level.

- **Effects:** Double the normal HP gained per level
- **Ranks:** 1
- **Requirements:** Strength 7, Endurance 5, Level 8  
- **Restrictions:** Only Deathclaws can choose this perk

---

### Cancerous Growth

Radiation has made you hardy.

- **Effects:**
  - +2 Healing Rate
  - Regenerate a crippled limb in 48 hours
- **Ranks:** 1
- **Requirements:** Strength < 7, Level 6  
- **Restrictions:** Only Ghouls can choose this perk

---

### Cautious Nature

You’re careful in strange situations.

- **Effects:** +3 Perception when determining starting position in random encounters
- **Ranks:** 1
- **Requirements:** Perception 6, Level 3

---

### Comprehension

You get more out of reading.

- **Effects:** Books grant +50% more skill points (e.g., 10 → 15)
- **Ranks:** 1
- **Requirements:** Intelligence 6, Level 3  
- **Restrictions:** Dogs cannot choose this perk

---

### Crazy Bomber

You’re unnaturally safe with explosives.

- **Effects:** If you fail to set an explosive properly, you instantly know and it will not detonate; you can reset and try again
- **Ranks:** 1
- **Requirements:** Traps 60%, Intelligence 6, Level 9  
- **Restrictions:** Dogs and Deathclaws cannot choose this perk

---

### Cult of Personality

Everyone likes you, regardless of karma.

- **Effects:** Karma modifiers for reactions are always positive; “wrong” karma type still gives a positive reaction bonus
- **Ranks:** 1
- **Requirements:** Charisma 10, Level 12

---

### Death Sense

You see well in darkness and sense danger.

- **Effects:**
  - +2 Perception in the dark
  - Light penalties reduced by 50%
  - +25% bonus to detecting sneaking enemies
- **Ranks:** (not specified in original text; treat as 1)
- **Requirements:** Intelligence 5, Level 4  
- **Restrictions:** Only Deathclaws can choose this perk

---

### Demolition Expert

You’re a professional with explosives.

- **Effects:**
  - Explosives you set deal +50% damage
  - Explosives always detonate on time
- **Ranks:** 1
- **Requirements:** Agility 4, Traps 90%, Level 9  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Die Hard

You stay up when others drop.

- **Effects:** When HP < 20% of maximum, +10% Damage Resistance to all damage types
- **Ranks:** 1
- **Requirements:** First Aid 40%, Endurance 6, Level 2

---

### Divine Favor

A higher power watches over you.

- **Effects:**
  - Once per 24 hours, you may re-roll any failed roll
  - You must accept the second result
- **Ranks:** 1
- **Requirements:** Charisma 8, Level 14

---

### Dodger

You’re harder to hit.

- **Effects:** +5 Armor Class per rank
- **Ranks:** 2
- **Requirements:** Agility 6, Level 9

---

### Drunken Master

You fight better when drunk.

- **Effects:** +20% Unarmed when under the influence of alcohol
- **Ranks:** 1
- **Requirements:** Unarmed 60%, Level 3

---

### Earlier Sequence

You move earlier in combat rounds.

- **Effects:** +2 Sequence per rank
- **Ranks:** 3
- **Requirements:** Perception 6, Level 3

---

### Educated

You learn more per level.

- **Effects:** +2 skill points per level, per rank
- **Ranks:** 3
- **Requirements:** Intelligence 6, Level 6

---

### Empathy

You get a read on people.

- **Effects:** GM must warn you when your dialogue choice will be interpreted negatively by an NPC
- **Ranks:** 1
- **Requirements:** Perception 7, Intelligence 5, Level 6

---

### Explorer

You find strange and interesting things.

- **Effects:** Increases chance to find special encounters and items (GM’s discretion)
- **Ranks:** 1
- **Requirements:** Level 9

---

### Faster Healing

You heal more quickly.

- **Effects:** +2 Healing Rate per rank
- **Ranks:** 3
- **Requirements:** Endurance 6, Level 3

---

### Flexible

You change stances quickly.

- **Effects:** Changing combat stance costs 1 AP
- **Ranks:** 1
- **Requirements:** Sneak 60%, Agility 6, Level 4

---

### Flower Child

You’re less affected by chems.

- **Effects:**
  - 50% less likely to become addicted
  - Withdrawal time is half normal
- **Ranks:** 1
- **Requirements:** Endurance 5, Level 9

---

### Fortune Finder

You find more money.

- **Effects:** Random encounters yield more currency (amount is GM’s discretion)
- **Ranks:** 1
- **Requirements:** Luck 8, Level 6

---

### Gain Agility / Charisma / Endurance / Intelligence / Luck / Perception / Strength

You permanently increase an attribute.

For each of these:

- **Effects:** +1 to the named Primary Statistic (permanently)
- **Ranks:** 1
- **Requirements:**  
  - Stat must be below racial maximum  
  - Level 12

---

### Gambler

You’re notably good at games of chance.

- **Effects:** One-time +20% to Gambling
- **Ranks:** 1
- **Requirements:** Gambling 50%, Level 6

---

### Ghost

You’re nearly invisible in the dark.

- **Effects:** +20% Sneak in darkness or at night
- **Ranks:** 1
- **Requirements:** Sneak 60%, Level 6

---

### Gunner

You’re good at firing from moving vehicles.

- **Effects:** No 10% penalty for shooting from a moving vehicle
- **Ranks:** 1
- **Requirements:** Guns 40%, Agility 6, Level 3

---

### Harmless

You look too innocent to be a thief.

- **Effects:** +20% Steal
- **Ranks:** 1
- **Requirements:** Steal 50%, Karma > 49, Level 6  
- **Restrictions:** Deathclaws cannot choose this perk

---

### Healer

You heal more HP with medical skills.

- **Effects:**
  - Rank 1: First Aid/Doctor heal an extra 1d6 + 4 HP
  - Rank 2: Heals an additional 2 × (1d6 + 4) HP
- **Ranks:** 2
- **Requirements:** Perception 7, Agility 6, Intelligence 5, First Aid 40%, Level 3

---

### Heave Ho!

You throw weapons farther.

- **Effects:** For thrown weapon range only, Strength is treated as +2 per rank
- **Ranks:** 3
- **Requirements:** Level 6

---

### Here and Now

You level up immediately.

- **Effects:** Gain enough XP to reach the next level immediately
- **Ranks:** 1
- **Requirements:** Level 9

---

### Hide of Scars

Your scarred hide is its own armor.

- **Effects:** +15% to all resistances except fire
- **Ranks:** 2
- **Requirements:** Endurance 6, Level 10  
- **Restrictions:** Only Deathclaws can choose this perk

---

### Hit the Deck!

You react quickly to explosives.

- **Effects:** Take half damage from ranged explosive weapons by increasing Explosive DR by 50%
- **Ranks:** 1
- **Requirements:** Agility 6, Level 4

---

### HtH Evade

Your unarmed stance improves your defense.

- **Effects:** If not holding any weapons at end of turn, gain 3 AC per unused AP instead of 1
- **Ranks:** 1
- **Requirements:** Unarmed 75%, Level 12

---

### Kama Sutra Master

You are exceptionally skilled in intimate matters.

- **Effects:** Great stamina and skill when “doing the dirty” (roleplay effect)
- **Ranks:** 1
- **Requirements:** Endurance 5, Agility 5, Level 3

---

### Karma Beacon

Your karma radiates outward.

- **Effects:** Karma is doubled for reaction purposes
- **Ranks:** 1
- **Requirements:** Charisma 6, Level 9

---

### Leadfoot

You drive faster.

- **Effects:** Vehicle maximum speed increased by 25%
- **Ranks:** 1
- **Requirements:** Pilot 60%, Perception 6, Agility 6, Level 3

---

### Leader

You inspire your party.

- **Effects (party members within 10 hexes):**
  - +1 Agility (up to racial max)
  - +5 Armor Class
- **Ranks:** 1
- **Requirements:** Charisma 6, Level 4  
- **Notes:** The leader does **not** gain these bonuses themselves

---

### Lifegiver

You gain extra HP on level up.

- **Effects:** +4 HP per level per rank
- **Ranks:** 2
- **Requirements:** Endurance 4, Level 12

---

### Light Step

You avoid triggers and traps.

- **Effects:** +4 effective Agility for trap triggering checks
- **Ranks:** 1
- **Requirements:** Agility 5, Luck 5, Level 9

---

### Living Anatomy

You understand where to hurt people.

- **Effects:**
  - +10% Doctor
  - +5 damage vs. living creatures on every attack
- **Ranks:** 1
- **Requirements:** Doctor 60%, Level 12

---

### Loner

You operate better alone.

- **Effects:** +10% to all skill rolls when at least 10 hexes away from all party members
- **Ranks:** 1
- **Requirements:** Outdoorsman 50%, Charisma < 5, Level 4

---

### Master Thief

You are a consummate thief.

- **Effects:** One-time +15% to Lockpick and Steal
- **Ranks:** 1
- **Requirements:** Lockpick 50%, Steal 50%, Level 12  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Master Trader

You are exceptionally good at trading.

- **Effects:** One-time +30% to Barter
- **Ranks:** 1
- **Requirements:** Charisma 7, Barter 60%, Level 9  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Medic

You are well-trained in medicine.

- **Effects:** One-time +10% to First Aid and Doctor
- **Ranks:** 1
- **Requirements:** First Aid 40% **or** Doctor 40%, Level 12

---

### Mental Block

You can tune out unwanted mental interference.

- **Effects:** For purposes of determining combat range and trap detection only, Perception is treated as +1 (up to racial max)
- **Ranks:** 1
- **Requirements:** Level 15

---

### More Criticals

You cause critical hits more often.

- **Effects:** +5% Critical Chance per rank
- **Ranks:** 3
- **Requirements:** Luck 6, Level 6  
- **Restrictions:** Mutants cannot choose this perk

---

### Mr. (or Ms.) Fixit

You excel at technical work.

- **Effects:** One-time +10% to Repair and Science
- **Ranks:** 1
- **Requirements:** Repair 40% **or** Science 40%, Level 12  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Mutate!

Your genetic profile changes.

- **Effects:**
  - Remove one existing Trait
  - Choose a new Trait
- **Ranks:** 1
- **Requirements:** Level 9

---

### Mysterious Stranger

A strange ally sometimes appears.

- **Effects:** On random encounters, 30% + (2 × Luck) chance to gain a temporary ally (GM chooses)
- **Ranks:** 1
- **Requirements:** Luck 4, Level 9

---

### Negotiator

You’re better at getting deals and persuading.

- **Effects:** One-time +10% to Speech and Barter
- **Ranks:** 1
- **Requirements:** Barter 50%, Speech 50%, Level 6  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Night Vision

You see better in the dark.

- **Effects:** Light penalties in darkness are reduced by 50%
- **Ranks:** 1
- **Requirements:** Perception 6, Level 3

---

### Pack Rat

You can carry more gear.

- **Effects:** +10 lbs Carry Weight per rank
- **Ranks:** 2
- **Requirements:** Level 6

---

### Pathfinder

You travel more efficiently.

- **Effects:** Overland travel time reduced by 25%
- **Ranks:** 1
- **Requirements:** Endurance 6, Outdoorsman 60%, Level 6

---

### Pickpocket

You’re very skilled at lifting items.

- **Effects:** +25% Steal when stealing from characters/NPCs
- **Ranks:** 1
- **Requirements:** Agility 8, Steal 80%, Level 15  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Presence

Your presence influences reactions.

- **Effects:** +1 Charisma for reaction rolls per rank
- **Ranks:** 3
- **Requirements:** Charisma 6, Level 6

---

### Psychotic

You’ve adapted to Psycho.

- **Effects:**
  - Positive effects of Psycho are doubled
  - Addiction rate for Psycho is halved
- **Ranks:** 1
- **Requirements:** Endurance 5, Level 8  
- **Restrictions:** Only mutants and half-mutants can choose this perk

---

### Pyromaniac

You do terrible things with fire.

- **Effects:** +5 damage with fire-based weapons (flamethrowers, molotovs, napalm, etc.)
- **Ranks:** 1
- **Requirements:** Guns 75%, Level 9

---

### Quick Pockets

You swap gear faster.

- **Effects:** Swapping equipment in combat costs 2 AP instead of 4
- **Ranks:** 1
- **Requirements:** Agility 5, Level 3

---

### Quick Recovery

You get up faster.

- **Effects:** Standing up from knockdown costs 1 AP
- **Ranks:** 1
- **Requirements:** Agility 5, Level 6

---

### Rad Child

Radiation heals you instead of harming you.

- **Effects:** When in a radiation source of 10+ rads/hour, +5 Healing Rate
- **Ranks:** 1
- **Requirements:** Endurance 6, Level 3  
- **Restrictions:** Only Ghouls can choose this perk

---

### Rad Resistance

You resist radiation better.

- **Effects:** +15% Radiation Resistance per rank
- **Ranks:** 2
- **Requirements:** Endurance 6, Intelligence 4, Level 6

---

### Ranger

You’re a seasoned wanderer.

- **Effects:**
  - +15% Outdoorsman
  - Special encounters and items are easier to find (GM’s discretion)
- **Ranks:** 1
- **Requirements:** Perception 6, Level 6

---

### Road Warrior

You can drive and fight at the same time.

- **Effects:** No penalties when driving and attacking simultaneously
- **Ranks:** 1
- **Requirements:** Pilot 60%, Intelligence 6, Level 12  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Salesman

You’re good at selling things.

- **Effects:** One-time +20% to Barter
- **Ranks:** 1
- **Requirements:** Barter 50%, Level 6  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Scout

You see farther and find more.

- **Effects:**  
  - Increased world map vision and map clarity  
  - Special encounters and items easier to find (GM’s discretion)
- **Ranks:** 1
- **Requirements:** Perception 7, Level 3

---

### Scrounger

You find more ammunition.

- **Effects:** Always find double the normal ammo in random encounters
- **Ranks:** 1
- **Requirements:** Luck 8, Level 9

---

### Sharpshooter

You’re better at long-range shooting.

- **Effects:** For range modifiers, Perception is treated as +2
- **Ranks:** 1
- **Requirements:** Perception 7, Intelligence 6, Level 9

---

### Silent Death

You excel at killing from the shadows.

- **Effects:** While successfully sneaking and attacking from behind with HtH or melee, you deal double damage
- **Ranks:** 1
- **Requirements:** Agility 10, Sneak 80%, Unarmed 80%, Level 18

---

### Silent Running

You can run while sneaking.

- **Effects:** Can run and sneak at the same time
- **Ranks:** 1
- **Requirements:** Agility 6, Sneak 50%, Level 6

---

### Slayer

You are a walking melee death machine.

- **Effects:** In HtH or melee, a successful attack that also succeeds on a Luck check is a critical hit
- **Ranks:** 1
- **Requirements:** Agility 8, Strength 8, Unarmed 80%, Level 24

---

### Smooth Talker

You sound smarter than you are (for talking).

- **Effects:** +1 Intelligence for “smooth-talking” rolls per rank
- **Ranks:** 3
- **Requirements:** Intelligence 4, Level 3  
- **Restrictions:** Dogs cannot choose this perk

---

### Snakeater

You shrug off poison.

- **Effects:** +25% Poison Resistance
- **Ranks:** 1
- **Requirements:** Endurance 3, Level 6

---

### Sniper

Your shots tend to be critical.

- **Effects:** With ranged weapons, successful attack rolls that also succeed on a Luck check are critical hits
- **Ranks:** 1
- **Requirements:** Agility 8, Perception 8, Guns 80%, Level 24  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Speaker

You’re especially persuasive.

- **Effects:** One-time +20% to Speech
- **Ranks:** 1
- **Requirements:** Speech 50%, Level 9  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Stat!

You’re faster at battlefield medicine.

- **Effects:** Using First Aid/Doctor to help someone in combat costs 5 AP
- **Ranks:** 1
- **Requirements:** First Aid 75%, Doctor 50%, Agility 6, Level 3

---

### Steady Arm

You handle burst fire better.

- **Effects:** Burst attacks cost 1 less AP
- **Ranks:** 1
- **Requirements:** Strength 6, Level 4  
- **Restrictions:** Only mutants can choose this perk

---

### Stonewall

You are hard to knock down.

- **Effects:** 50% chance to avoid being knocked down in combat
- **Ranks:** 1
- **Requirements:** Strength 6, Level 3

---

### Strong Back

You can carry significantly more.

- **Effects:** +50 lbs Carry Weight per rank
- **Ranks:** 2
- **Requirements:** Strength 6, Endurance 6, Level 3

---

### Stunt Devil

You handle impacts better.

- **Effects:**
  - 25% less damage from falls and vehicle wrecks
  - +10% Pilot
- **Ranks:** 2
- **Requirements:** Strength 6, Endurance 6, Agility 6, Level 6  
- **Restrictions:** No stunt dogs (Dogs cannot choose this perk)

---

### Survivalist

You’re excellent at survival.

- **Effects:** +25% Outdoorsman per rank
- **Ranks:** 3
- **Requirements:** Endurance 6, Intelligence 6, Outdoorsman 40%, Level 3

---

### Swift Learner

You gain more experience.

- **Effects:** +5% XP gain per rank (rounded up)
- **Ranks:** 3
- **Requirements:** Intelligence 4, Level 3

---

### Tag!

You get an extra tag skill.

- **Effects:** Choose one additional Tag Skill
- **Ranks:** 1
- **Requirements:** Level 12

---

### Talon of Fear

Your claws are venomous.

- **Effects:** All unarmed attacks inflict Type B poison
- **Ranks:** 1
- **Requirements:** Unarmed 60%, Strength 6, Level 12  
- **Restrictions:** Only Deathclaws can choose this perk

---

### Team Player

You work best with the group.

- **Effects:** When all party members are within 10 hexes, you gain +10% to all skills
- **Ranks:** 1
- **Requirements:** Charisma 4, Level 12

---

### Thief

You are broadly skilled in thievery.

- **Effects:** One-time +10% to Sneak, Lockpick, Steal, and Traps
- **Ranks:** 1
- **Requirements:** Level 3  
- **Restrictions:** Dogs and Deathclaws cannot choose this perk

---

### Tough Hide

Your hide is hardened by the wastes.

- **Effects:**
  - +15 Armor Class
  - +10% to all resistances
- **Ranks:** 2
- **Requirements:** Endurance < 8, Level 12  
- **Restrictions:** Only mutants can choose this perk

---

### Toughness

You are naturally more resilient.

- **Effects:** +10% Damage Resistance to all damage types
- **Ranks:** 1
- **Requirements:** Endurance 6, Luck 6, Level 3

---

### Tunnel Rat

You move quickly while low.

- **Effects:** Move at normal rate (1 AP per hex) while crouching or prone
- **Ranks:** 1
- **Requirements:** Sneak 60%, Agility 6, Level 4

---

### Way of the Fruit

Fruit has mystical effects on you.

- **Effects:** For 24 hours after eating fruit, +1 Perception and +1 Agility (respecting racial max/min)
- **Ranks:** 1
- **Requirements:** Charisma 6, Level 6  
- **Restrictions:** Deathclaws and Dogs cannot choose this perk

---

### Weapon Handling

You handle heavy weapons more easily.

- **Effects:** +3 effective Strength for meeting weapon Strength requirements
- **Ranks:** 1
- **Requirements:** Agility 5, Level 12

---

## Implementation Notes

- Perk eligibility must be checked against:
  - Level
  - Attributes
  - Skills
  - Race / type
- Perk ranks must never exceed the listed maximum
- Robots **never** gain perks
- Racial and special restrictions must be strictly enforced
- Effects that alter fundamental rules (like action costs, critical behavior, or XP gain) should be implemented in core logic, not as simple modifiers

This document is the authoritative reference for player perks and advancement.
