...
// ─── GM Password Protection ───────────────────────────────────────────────────
// Default password: "gm" (can be changed via browser console: window.GM_PASSWORD = 'newpass')
window.GM_PASSWORD = window.GM_PASSWORD || 'lightning';
window.GM_SESSION_KEY = 'gm_session_authenticated';
...
function initPasswordProtection() {
  ...
  const gmRoot = document.getElementById('gm-root');

  // Check if already authenticated in this session
  if (sessionStorage.getItem(window.GM_SESSION_KEY)) {
    if (modal) modal.style.display = 'none';
    if (gmRoot) gmRoot.classList.remove('locked');
    return;
  }

  function validatePassword() {
    const enteredPassword = input.value.trim();
    if (enteredPassword === window.GM_PASSWORD) {
      // Authentication successful
      sessionStorage.setItem(window.GM_SESSION_KEY, 'true');
      if (modal) modal.style.display = 'none';
      if (gmRoot) gmRoot.classList.remove('locked');
